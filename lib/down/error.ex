defmodule Down.Error do
  @moduledoc """
  Represents an error with the transport used by an HTTP connection.

  A `Down.Error` struct is an exception, so it can be raised as any
  other exception.
  ...
  for using in logs), you can use `Exception.message/1`:
      ...
      iex> {:error, %Down.Error{} = error} = Down.read("http://nonexistent", 80)
  ...
  defexception [:reason, :custom_message]

  def message(%__MODULE__{custom_message: msg}) when not is_nil(msg) do
    msg
  end
  ...
  defp format_reason(:invalid_headers) do
    "invalid headers"
  end

  defp format_reason(:invalid_method) do
    "invalid method"
  end
